...
#ifdef _WIN32
#define FILE_BYTE_ORDER little
#elif defined(__BYTE_ORDER__) && defined(__ORDER_LITTLE_ENDIAN__) && defined(__ORDER_BIG_ENDIAN__)
#if __BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__
#define FILE_BYTE_ORDER little
#elif __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
#define FILE_BYTE_ORDER big
#endif
#endif
...
template <class T, endian Endianness = endian::native, size_t Size = sizeof(T)>
...
inline constexpr T byteswap(T value) noexcept
{
  ...
  else if constexpr(sizeof(T) == sizeof(uint32_t))
  {
    ...
    {
      return bit_cast<T>(byteswap32(bit_cast<uint32_t>(value)));
    }
    else
    {
